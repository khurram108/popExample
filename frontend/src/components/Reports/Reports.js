...
import { getAllReport } from '../../redux/actions/Statistics'
...
const columns = [
    { id: 'country', label: `Date`, minWidth: 170 },
...
function Billings(props) {
    ...
    const [selectedWebiste, setSelectedWebsite] = React.useState(null);
    ...
    useEffect(() => {
        getAllReport({ user_id: user.uuid })
        getAllWebsites({ userID: user._id })
    }, [])

    useEffect(() => {
        setStatistics(statistics.allReports)
        setStatistics2(statistics.allReports)
        setAllWebsites(websites.allWebsites)


    }, [statistics.allReports, websites.allWebsites])
    ...
    const clickToGenerate = e => {
        ...
        if (!country && website == "Select Website" && (day == "5 days" || day === null)) {
            console.log("Works");
            setStatistics(stats2)
            return true
        }
        if (website && country && day) {

            filterByThree(website, country, day)

        }
        if (website && country) {
            let array = stats2.filter((item) => {
                return item.country == country && item.domain_uuid == website
            })
            setStatistics(array)
        }
        ...
        if (country && day) {
            filterCountry_day(country, day)
        }

        if (country && !website && !day) {
            let array = stats2.filter((item) => {
    ...
    const filterByThree = (website, country, day) => {
        ...
        if (day == "today") {
            ...
            array = stats2.filter((item) => {
                ...
                return date == fullDate && item.domain_uuid == website && item.country == country
            })
    ...
    const filterWeb_Domain = (website, day) => {
        ...
        if (day == "today") {
            ...
            array = stats2.filter((item) => {
                let date = item.created_at.slice(0, 10)
                return date == fullDate && item.domain_uuid == website
            })
    ...
    const filterCountry_day = (country, day) => {
        ...
        if (day == "today") {
            ...
            array = stats2.filter((item) => {
                let date = item.created_at.slice(0, 10)
                return date == fullDate && item.country == country
            })
                                        ...
                                        <Table stickyHeader aria-label="sticky table" >
                                            <TableHead>
                                                <TableRow>
                                                    ...
                                                    <TableCell align="center" className={classes.borderRight}>Country <List className={classes.listIcon} /></TableCell>
                                                    <TableCell align="center" className={classes.borderRight}>Impressions <List className={classes.listIcon2} /></TableCell>
                                                    <TableCell align="center" className={classes.borderRight}>eCPM <List className={classes.listIcon3} /></TableCell>
                            ...
                            <div class="intro-y box">
                                ...
                                <div class="p-5" id="vertical-form">
                                    <div class="preview">
                                        ...
                                        <div className="mt-5">
                                            <label class="py-5">Report By Country</label>
...
export default connect(mapStateToProps, { getAllReport, earningStatus, getAllWebsites })(Billings);